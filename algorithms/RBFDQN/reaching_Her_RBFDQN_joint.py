...
if __name__ == '__main__':
    ...
    if is_testing == 'testing':
        ...
        Q_object.load_state_dict(torch.load('obj_net'))
        Q_object_target.load_state_dict(torch.load('obj_target_net'))
        Q_object.eval()
        Q_object_target.eval()
    ...
    for episode in range(params['max_episode']):
        ...
        while not done:
            ...
            # new sp is [arm.get_joint_positions(), tip.get_pose(), target.get_position()] 
            sp = [*sp[8:15], *sp[22:29], *sp[-3:]]
            ...
            temp_buffer.append((s, a, r, done_p, sp))

            s = sp
            
            # track how many steps have passed
            if t % 100 == 0:
            	print('number of steps: ', t)

        #naive hindsight experience reply
        if not success:
            #if we haven't reached the target, we create an imagined trajectory in which the target is at the last state of the arm tip's position
            target_position_her = tip.get_position()
            for i in range(len(temp_buffer)):
                s_her, a, r_her, done, sp_her = temp_buffer[i]
                s_her[-3:] = target_position_her
                sp_her[-3:] = target_position_her
                if i == len(temp_buffer)-1:
                    r_her = 1
                    done = True
                Q_object.buffer_object.append(s_her, a, r_her, done, sp_her)
        ...
        #update networks if we are not testing
        if is_testing != 'testing':
        ...
        if (episode % 10 == 0) or (episode == params['max_episode'] - 1):
            ...
            if numpy.mean(temp) ==1:
                torch.save(Q_object.state_dict(), 'obj_net')
                torch.save(Q_object_target.state_dict(), 'obj_target_net')
