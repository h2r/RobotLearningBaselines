...
class BehaviorCloneLoss(nn.Module):
    ...
    def forward(self, out, target, flag=False): # aux_out, aux_target,
        ...
        cout = out[:, :3]
        ctarget = target[:, :3]
        num = torch.bmm(ctarget.view(bs,1,3), cout.view(bs,3,1)).squeeze()
        ...
        if flag:#torch.isnan(weighted_loss):
            ...
            print('weighted loss: %.5f' % weighted_loss)
            print('l2 loss: %.4f' % l2_loss)
            print('l1 loss: %.4f' % l1_loss)
